...
namespace Montesino_fakture.View
{
    public partial class Valute : Form
    {
        ...
        public void ucitajTabelu()
        {
            try
            {
                DataTable table = MainForm.getData.GetTableValute();
                ...
                gridView.Columns["Oznaka"].Caption = "Oznaka";
        ...
        private void gridView_RowUpdated(object sender, DevExpress.XtraGrid.Views.Base.RowObjectEventArgs e)
        {
            try
            {
                if (gridView.IsNewItemRow(e.RowHandle) == false) //IZMENA
                {
                    ...
                    {
                        var jm_id = Convert.ToInt32(red["Valuta_ID"]);
                        var jm = con.Valutas.SingleOrDefault(x => x.Valuta_ID == jm_id);

                        jm.Oznaka = red["Oznaka"].ToString().Trim();
        ...
        private void gridView_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            try
            {
                ...
                var Oznaka = gridView.GetFocusedRowCellValue("Oznaka").ToString().Trim();
                var Naziv = gridView.GetFocusedRowCellValue("Naziv").ToString().Trim();
                Boolean napravi = true;

                if (Oznaka == "" || Oznaka.Length > 3)
                {
                    e.Valid = false;
                    view.SetColumnError(gridView.Columns["Oznaka"], "[MAX: 3 KARAKTERA]: Polje ne sme biti prazno!");
                    napravi = false;
                }
                ...
                if (gridView.IsNewItemRow(e.RowHandle) && napravi == true) //DODAVANJE
                {
                    ...
                    using (var con = new MONTESINOEntities())
                    {
                        var jm = new Model.Valuta()
                        {
                            Oznaka = red["Oznaka"].ToString().Trim(),
                            Naziv = red["Naziv"].ToString().Trim(),
                        };
                        con.Valutas.Add(jm);
                        con.SaveChanges();
                        red["Valuta_ID"] = jm.Valuta_ID;
                    }
                }
            }
            catch (DbUpdateException ex)
        ...
        private void gridView_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.KeyCode == Keys.Delete && e.Modifiers == Keys.Control && red != null)
                {
                    ...
                    {
                        var jm_id = Convert.ToInt32(red["Valuta_ID"]);
                        con.Valutas.RemoveRange(con.Valutas.Where(x => x.Valuta_ID == jm_id));
                        con.SaveChanges();
                        GridView view = sender as GridView;
